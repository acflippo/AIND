...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # list of new action level nodes
        a_nodes = set()

        for action in self.all_actions:

            # Create a new Planning Graph Node for this action
            node_action = PgNode_a(action)

            # Test if the new action nodes has prenodes that is a subset of the previous state level
            # If yes, add it to the list of action levels,
            #         connect this action node as a child of the previous state node, and
            #         connect this state node as a parent of the previous action node.
            if node_action.prenodes.issubset(self.s_levels[level]):
                a_nodes.add(node_action)

                for node_state in self.s_levels[level]:
                    node_state.children.add(node_action)
                    node_action.parents.add(node_state)

        # Add action level to Planning Graph
    ...
    def add_literal_level(self, level):
        ...
        # Every action node in the previous level
        for node_action in self.a_levels[level-1]:

            # Add every effnodes to the new S level
            for node_state in node_action.effnodes:
                s_nodes.add(node_state)
                # add the action node to the parent set of the state node
                node_state.parents.add(node_action)
                # add the state node to the children of the action node
                node_action.children.add(node_state)

        # add state level to the Planning Graph
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check to see if the action of node_a1 is in the effects of node_a2
        # If yes, then one node_a1's action negates the node_a2's effects
        if set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_rem)):
            return True

        # Check to see if the action of node_a2 is in the effects of node_a1
        if set(node_a2.action.effect_add).intersection(set(node_a1.action.effect_rem)):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_a1_add = set(node_a1.action.effect_add)
        node_a2_add = set(node_a2.action.effect_add)

        node_a1_rem = set(node_a1.action.effect_rem)
        node_a2_rem = set(node_a2.action.effect_rem)

        node_a1_pos = set(node_a1.action.precond_pos)
        node_a2_pos = set(node_a2.action.precond_pos)

        node_a1_neg = set(node_a1.action.precond_neg)
        node_a2_neg = set(node_a2.action.precond_neg)

        # Check if effect of one action negates the precondition of the other
        if node_a1_add.intersection(node_a2_neg):
            return True

        if node_a2_add.intersection(node_a1_neg):
            return True

        if node_a1_rem.intersection(node_a2_pos):
            return True

        if node_a2_rem.intersection(node_a1_pos):
            return True
